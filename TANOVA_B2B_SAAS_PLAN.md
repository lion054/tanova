# Tanova Travel B2B SaaS Platform

## Branding

**Tanova** is the brand for this B2B SaaS travel platform.

Logo assets live in [assets/branding/](assets/branding/):

| Asset | File | Use |
|-------|------|-----|
| Wordmark (black) | [tanova-logo-black.png](assets/branding/tanova-logo-black.png) | Light backgrounds |
| Wordmark (white) | [tanova-logo-white.png](assets/branding/tanova-logo-white.png) | Dark backgrounds |
| Mark / icon | [tanova-mark.png](assets/branding/tanova-mark.png) | Favicon, app icon, compact spaces |

The mark is the swoosh "N" device; the wordmark pairs it with the Tanova type.
...
```
┌─────────────────────────────────────────────────────┐
│ Tanova SaaS Platform (tsokaupgrade)                 │
│                                                      │
│ • Marketing website                                 │
...
└─────────────────────────────────────────────────────┘
                         ↓
┌─────────────────────────────────────────────────────┐
│ Tanova Multi-Tenant Backend (gotrip/bc-cms)         │
│                                                      │
│ Core Booking System:                                │
...
### Phase 1: Add SaaS Infrastructure to Tanova Backend

**Goal:** Make Tanova multi-tenant with API keys
...
3. **Integrate with Tanova API**
   - API client wrapper
   - Proxy requests to Tanova backend
...
### Phase 1A: API Key System in Tanova
...
│       └── proxy/[...path]/    ← Proxy to Tanova backend
├── components/
│   ├── dashboard/
│   │   ├── ApiKeyManager.tsx
...
│   ├── api-client.ts        ← Tanova API wrapper
│   └── constants.ts
└── package.json
```
...
├── .env.example
│   VITE_TANOVA_API=https://gotrip.tsokatravel.com
│   VITE_OPERATOR_API_KEY=<tenant_api_key>
│   VITE_SITE_SLUG=dare2travel
├── src/
│   ├── App.tsx
│   ├── services/
│   │   └── tanovaApi.ts      ← Calls Tanova API
│   ├── pages/
│   │   ├── Home.tsx
│   │   ├── Hotels.tsx
│   │   ├── Tours.tsx
...
|-------|----------|-------------|
| **1: API Keys + Tenant Isolation** | 2-3 weeks | Tanova is multi-tenant, supports API keys, rate limiting |
...
3. **Multi-Vendor Marketplace** — Already built into Tanova
...
**Immediate:**
1. Audit Tanova's existing Vendor module
