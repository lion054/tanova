...
<head>
    ...
    <title>@yield('title', 'Error') — {{ config('app.name', 'Tsoka Travel') }}</title>
...
<nav class="err-nav">
    <a class="err-nav-brand" href="{{ url('/') }}">
        @php $logo = setting_item('logo_id') ? get_file_url(setting_item('logo_id')) : null; @endphp
        @if($logo)
            <img src="{{ $logo }}" alt="{{ setting_item('site_title', 'Tsoka Travel') }}">
        @else
            ...
            {{ setting_item('site_title', 'Tsoka Travel') }}
        @endif
    </a>
    <a class="err-nav-home" href="{{ url('/') }}">
...
<div class="err-stage">
    <div class="err-bg-code" aria-hidden="true" style="color:@yield('code-color','#1d4ed8')">@yield('code','ERR')</div>
    <div class="err-card">
        <div class="err-icon-wrap" style="background:@yield('icon-bg','#eff6ff')">
            @yield('icon')
        </div>
        <span class="err-badge" style="background:@yield('badge-bg','#eff6ff');color:@yield('badge-color','#1d4ed8')">@yield('badge-label','Error')</span>
        <div class="err-code">@yield('code','?')</div>
        <h1 class="err-title">@yield('title','Something went wrong')</h1>
        <p class="err-message">@yield('message','An unexpected error occurred.')</p>
        <div class="err-actions">@yield('actions')</div>
    </div>
</div>

<footer class="err-footer">
    &copy; {{ date('Y') }} {{ setting_item('site_title', 'Tsoka Travel') }}. All rights reserved.
</footer>
