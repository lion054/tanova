...
<?php $__env->startSection('content'); ?>
    <div class="container-fluid" id="menu-app" data-id="<?php echo e($row->id ?? ''); ?>" v-cloak>
        ...
        <div class="alert" v-show="message.content" v-bind:class="message.type ? 'alert-success' : 'alert-danger'">
            {{message.content}}
        </div>
        ...
        <div class="row">
            <div class="col-md-4 col-xl-3 menu-item-types">
                <div v-for="(type,index) in item_types" class="panel panel-toggle-able">
                    <div class="panel-title" v-on:click="type.open = type.open ? false : true">{{type.name}}
                    ...
                    <div class="panel-body" v-show="type.open">
                        ...
                        <div class="list-scrollable" v-show="type.items.length">
                            <div v-for="item in type.items"><label><input v-model="type.selected" type="checkbox"
                                                                          v-bind:value="item.id"> {{item.name}}</label></div>
                        </div>
                        <div class="alert-text danger mt10" v-show="!type.items.length"><?php echo e(__("No items found")); ?></div>
                        <div class="text-right">
                            <span class="btn btn-sm btn-primary" v-on:click="addToMenu(type)"><?php echo e(__('Add to Menu')); ?></span>
                        </div>
                    </div>
                </div>
                <div class="panel panel-toggle-able">
                    <div class="panel-title" v-on:click="custom_show = custom_show ? false : true"><?php echo e(__('Custom Url')); ?>
                    ...
                    <div class="panel-body" v-show="custom_show">
                        ...
                        <div class="text-right">
                            <span class="btn btn-sm btn-primary" v-on:click="addCustomUrl"><?php echo e(__('Add to Menu')); ?></span>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="lang-content-box">
                    ...
                    <div class="panel-body">
                        <div class="menu-items-zone">
                            <Draggable ref="tree" v-model="items" draggable cross-tree v-bind:triggerClass="triggerClass">
                                <template #default="{ node, stat }"  >
                                    <div class="nestable-item-content">
                                        <div class="menu-title drag-trigger">{{node.name}}
                                            ...
                                            <i class="icon ion-md-arrow-dropdown" v-on:click="toogleItem(node)"></i>
                                            </span>
                                        </div>
                                        <div class="menu-info" v-show="node._open">
                                            ...
                                            <div class="d-flex justify-content-between">
                                                <a href='#' v-on:click.prevent="deleteMenuItem(stat)"
                    ...
                    <div class="panel-footer text-right">
                        <span class="alert-text" v-show="message.content" v-bind:class="message.type ? 'success' : 'danger'">{{message.content}} &nbsp;</span>
                        <span class="btn btn-success" v-on:click="saveMenu"><?php echo e(__("Save Menu")); ?></span>
                    </div>
                </div>
                </div>
            </div>
    ...
    <style>
        .vtlist-inner .tree-node.drag-placeholder-wrapper{
            margin-top: 10px;
        }
    </style>
<?php $__env->stopSection(); ?>

<?php echo $__env->make('admin.layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /home/lionel/Documents/Junkyard/gotrip/bc-cms/modules/Core/Views/admin/menu/detail.blade.php ENDPATH**/ ?>