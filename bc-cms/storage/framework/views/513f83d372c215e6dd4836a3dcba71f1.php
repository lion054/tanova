<div>
                ...
                <div class="col-md-3">
                    <div class="nav flex-column nav-pills" id="v-pills-tab" role="tablist" aria-orientation="vertical">
                        <?php $__currentLoopData = $this->steps; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $step): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <button class="border-0 nav-link <?php echo e($index == $this->stepIndex ? 'active' : ''); ?> mb-2"
                                <?php if($index > $this->maxStepIndex): ?> disabled <?php elseif($index !== $this->stepIndex): ?> wire:click="setStep('<?php echo e($index); ?>')" <?php endif; ?>
                                id="<?php echo e($step['id']); ?>-tab" type="button" role="tab"
                ...
                <div class="col-md-9">
                    <?php if($this->currentStep): ?>
                        ...
                        <div class="d-flex justify-content-center">
                            <button
                                class="button h-60 px-24 -dark-1 bg-blue-1 text-white"><?php echo e(__('Next Step')); ?></button>
                        </div>
                    <?php endif; ?>
                </div>
            </div>
                        ...
                        success: (res) => {
                            ...
                            if (res.data) {
                                ...
                                if (res.data.is_image) {
                                    div.append("<img style='max-width: 100%; height:auto;' src='" + res
                                        .data.download + "' alt='" +
                                        res.data.name + "'>");
                                    div.append("<a target='_blank' href='" + res.data.download + "'> " +
                                        res.data.name + '.' + res
                                        .data.file_extension +
                                        " <i class=\"fa fa-download\"></i> </a>");
                                }

                                const keys = ['name', 'path', 'file_extension', 'file_type', 'size',
                                    'driver'
                                ];
                                ...
                                if (me.data('multiple')) {
                                    ...
                                    $wire.set('data.' + fieldId, [...old, JSON.stringify(dataToSave)],
                                        false);
                                } else {
                                    lists.html(div);
    ...
    ?>
<?php /**PATH /home/lionel/Documents/Junkyard/gotrip/bc-cms/themes/GoTrip/Form/Views/frontend/simple-form.blade.php ENDPATH**/ ?>