...
<div class="container-fluid">
    ...
    <nav class="mb-3"><ol class="breadcrumb small mb-0">
        ...
        <li class="breadcrumb-item active"><?php echo e($conversation->guestDisplayName()); ?></li>
    </ol></nav>

    <div class="d-flex justify-content-between align-items-start mb-4 flex-wrap gap-3">
        <div>
            <p class="portal-eyebrow">Conversation</p>
            <h1 class="portal-h1"><?php echo e($conversation->guestDisplayName()); ?></h1>
            <div style="display:flex;align-items:center;gap:8px;margin-top:6px;flex-wrap:wrap">
                <?php if($conversation->status === 'open'): ?>
                    <span class="tp-badge tp-badge--green">Open</span>
                <?php elseif($conversation->status === 'escalated'): ?>
                ...
                ?>
                <span class="tp-badge <?php echo e($chBadge); ?>"><?php echo e(ucfirst($conversation->channel)); ?></span>
            </div>
        </div>
        <div class="d-flex gap-2 flex-wrap">
            ...
            <?php if($conversation->status === 'open'): ?>
            <form method="POST" action="<?php echo e(route('admin.concierge.escalate', $conversation)); ?>" style="display:inline">
                <?php echo csrf_field(); ?>
                <button class="tp-ab-btn tp-ab-btn--danger">
                    <i class="ion ion-ios-warning"></i> Escalate
                </button>
            </form>
            <form method="POST" action="<?php echo e(route('admin.concierge.resolve', $conversation)); ?>" style="display:inline">
                <?php echo csrf_field(); ?>
        ...
        <div class="col-md-8">
            <div class="tp-card">
                <div class="tp-card-header">
                    ...
                    <span style="font-size:11px;color:#aaa;font-weight:400;margin-left:auto">
                        <?php echo e($conversation->messages->count()); ?> message<?php echo e($conversation->messages->count() !== 1 ? 's' : ''); ?>
                ...
                <div class="cc-thread" id="cc-thread">
                    ...
                    ?>
                    <div class="cc-msg cc-msg--<?php echo e($type); ?>">
                        <div class="cc-avatar cc-avatar--<?php echo e($type); ?>">
                            <?php if($type === 'guest'): ?>  G
                            <?php elseif($type === 'ai'): ?> AI
                            <?php else: ?>                   A
                            <?php endif; ?>
                        </div>
                        <div>
                            <div class="cc-bubble cc-bubble--<?php echo e($type); ?>">
                                <?php echo e($msg->body); ?>

                                <?php if($msg->isAiDraft() && !$msg->approved): ?>
                            ...
                            <div class="cc-meta">
                                ...
                                <?php if($type === 'ai'): ?> · AI draft <?php endif; ?>
                            </div>
                        </div>
                    </div>
                ...
                <?php if($conversation->status === 'open'): ?>
                <div class="cc-reply">
                    <form method="POST" action="<?php echo e(route('admin.concierge.reply', $conversation)); ?>">
                        <?php echo csrf_field(); ?>
                        <textarea name="body" placeholder="Type your reply to <?php echo e($conversation->guestDisplayName()); ?>…"
                                  required></textarea>
                ...
                <div style="padding:14px 20px;background:#fafafa;border-top:1px solid #f0f0f0;font-size:12px;color:#aaa;text-align:center">
                    Conversation <?php echo e($conversation->status); ?>. Reopen to reply.
                </div>
                <?php endif; ?>
            </div>

            
            <?php if(isset($aiDraft) && $aiDraft && $conversation->status === 'open'): ?>
            <div class="cc-ai-draft">
                ...
                <div class="cc-ai-draft-body">
                    <form method="POST" action="<?php echo e(route('admin.concierge.approve-ai', $conversation)); ?>">
                        <?php echo csrf_field(); ?>
                        <textarea name="body" rows="4"><?php echo e($aiDraft); ?></textarea>
        ...
        <div class="col-md-4">
            <div class="tp-card" style="margin-bottom:14px">
                ...
                <div class="tp-card-body" style="padding:16px 20px">
                    <table class="cc-info">
                        <tr><td>Name</td><td><?php echo e($conversation->guestDisplayName()); ?></td></tr>
                        <tr><td>Email</td><td><?php echo e($conversation->guest_email ?: '—'); ?></td></tr>
                        <tr><td>Channel</td><td><?php echo e(ucfirst($conversation->channel)); ?></td></tr>
                        <tr><td>Status</td><td>
                            <?php if($conversation->status === 'open'): ?>
                                <span class="tp-badge tp-badge--green">Open</span>
                            <?php elseif($conversation->status === 'escalated'): ?>
                        ...
                        <tr><td>Started</td><td><?php echo e($conversation->created_at?->format('d M Y H:i')); ?></td></tr>
                        <?php if($conversation->booking_id): ?>
                        <tr><td>Booking</td><td>
                            <a href="<?php echo e(route('admin.booking.detail', $conversation->booking_id)); ?>"
                               style="color:#2563eb;font-weight:600">#<?php echo e($conversation->booking_id); ?></a>
                        </td></tr>
                        <?php endif; ?>
                    </table>
                </div>
            ...
            <div class="tp-card" style="border-color:#fde68a;background:#fffbeb">
                <div class="tp-card-body" style="padding:14px 18px">
                    ...
                    <div class="d-flex flex-column gap-2">
                        <form method="POST" action="<?php echo e(route('admin.concierge.escalate', $conversation)); ?>">
                            <?php echo csrf_field(); ?>
                        ...
                        <form method="POST" action="<?php echo e(route('admin.concierge.resolve', $conversation)); ?>">
                            <?php echo csrf_field(); ?>
...
<?php echo $__env->make('layouts.user', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /home/lionel/Documents/Junkyard/gotrip/bc-cms/pro/Concierge/Views/admin/conversation.blade.php ENDPATH**/ ?>