...
<div class="container-fluid">
    ...
    <div class="d-flex justify-content-between align-items-start mb-4 flex-wrap gap-3">
        ...
        <form method="post" action="<?php echo e(route('booking.admin.bulkEdit')); ?>" id="bulk-form" style="display:flex;gap:8px;align-items:center">
            <?php echo csrf_field(); ?>
            <select name="action" class="tp-filter-field" style="height:38px;padding:0 12px;border:1.5px solid #e4e4e4;border-radius:8px;font-size:13px;color:#333;background:#fff;outline:none">
                ...
                <?php if(!empty($statues)): ?>
                    <?php $__currentLoopData = $statues; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $status): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <option value="<?php echo e($status); ?>">Mark as: <?php echo e(booking_status_to_text($status)); ?></option>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                <?php endif; ?>
                <option value="delete">DELETE selected</option>
            </select>
            <button data-confirm="<?php echo e(__('Do you want to delete?')); ?>" class="tp-ab-btn tp-ab-btn-outline dungdt-apply-form-btn" type="button">Apply</button>
        </form>
        <?php endif; ?>
    </div>
    ...
    <div class="row g-3 mb-4">
        <div class="col-6 col-md-3">
            <div class="tp-stat">
                ...
                <div class="tp-stat-amount"><?php echo e($totalCount); ?></div>
                <div class="tp-stat-sub"><?php echo e($cancelled); ?> cancelled</div>
            </div>
        </div>
        <div class="col-6 col-md-3">
            <div class="tp-stat">
                <div class="tp-stat-label">Total Revenue</div>
                <div class="tp-stat-amount"><?php echo e(format_money_main($totalRev)); ?></div>
        ...
        <div class="col-6 col-md-3">
            <div class="tp-stat">
                ...
                <div class="tp-stat-amount" style="color:#16a34a"><?php echo e(format_money_main($totalPaid)); ?></div>
                <div class="tp-stat-sub"><?php echo e(format_money_main($totalRev - $totalPaid)); ?> remaining</div>
            </div>
        </div>
        <div class="col-6 col-md-3">
            <div class="tp-stat">
                <div class="tp-stat-label">Awaiting Action</div>
                <div class="tp-stat-amount" style="color:#d97706"><?php echo e($processing); ?></div>
        ...
        <div class="tp-card mb-4">
            <div class="tp-filter-bar">
                ...
                <div class="tp-filter-field flex-grow-1" style="max-width:320px">
                    ...
                    <input type="text" name="s" value="<?php echo e(Request()->s); ?>" placeholder="Search by name, email or ID…">
                </div>
                <button type="submit" class="tp-ab-btn tp-ab-btn-primary">
                    <i class="ion ion-ios-funnel"></i> Filter
                </button>
                <?php if(Request()->s || Request()->vendor_id): ?>
                    <a href="<?php echo e(route('booking.admin.index')); ?>" class="tp-ab-btn tp-ab-btn-outline">Clear</a>
                <?php endif; ?>
                <span style="margin-left:auto;font-size:12px;color:#aaa;align-self:center">
                    <i><?php echo e($rows->total()); ?> booking(s) found</i>
                </span>
            </div>
        </div>
    </form>
                        ...
                        <tr>
                            ...
                            <td>
                                <?php if($service = $row->service): ?>
                                    <a href="<?php echo e($service->getDetailUrl()); ?>" target="_blank" class="bk-service-name">
                                        <?php echo e(Str::limit($service->title ?? '', 45)); ?>

                                    </a>
                                    <?php if($row->vendor): ?>
                                    <div class="bk-meta">
                                        by <a href="<?php echo e(route('user.admin.detail',['id'=>$row->vendor_id])); ?>" style="color:#2563eb" target="_blank">
                                            <?php echo e($row->vendor->name_or_email); ?>
                            ...
                            <td>
                                <div class="bk-customer-name"><?php echo e($row->first_name); ?> <?php echo e($row->last_name); ?></div>
                                <div class="bk-customer-meta">
                                    <?php if($row->email): ?><a href="mailto:<?php echo e($row->email); ?>" style="color:#2563eb"><?php echo e($row->email); ?></a><br><?php endif; ?>
                                    <?php if($row->phone): ?><?php echo e($row->phone); ?><?php endif; ?>
                                </div>
                            </td>
                            <td>
                                <div class="bk-money-total"><?php echo e(format_money_main($row->total)); ?></div>
                            ...
                            <td>
                                ...
                                ?>
                                <span class="tp-badge tp-badge-<?php echo e($sc); ?>"><?php echo e($row->statusName); ?></span>
                            </td>
                            ...
                            <td style="font-size:12px;color:#aaa;white-space:nowrap">
                                <?php echo e(display_datetime($row->updated_at)); ?>
                                ...
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-right" style="min-width:160px;border:1px solid #ebebeb;border-radius:8px;box-shadow:0 8px 24px rgba(0,0,0,.1);padding:4px">
                                        <a class="dropdown-item btn-detail-booking"
                                           ...
                                           data-ajax="<?php echo e(route('booking.modal',['booking'=>$booking])); ?>"
                                           data-toggle="modal"
                                        ...
                                        <a class="dropdown-item" href="#"
                                           data-toggle="modal" data-target="#modal-paid-<?php echo e($row->id); ?>"
                                           style="font-size:13px;padding:8px 14px;border-radius:6px">
                                            <i class="ion ion-ios-card" style="width:18px"></i> Set Paid
                                        </a>
                                        <a class="dropdown-item"
                                           href="<?php echo e(route('booking.admin.email_preview',['id'=>$row->id])); ?>"
                                           target="_blank"
                                           style="font-size:13px;padding:8px 14px;border-radius:6px">
                                            <i class="ion ion-ios-mail" style="width:18px"></i> Email Preview
                                        </a>
                                        <?php if(auth()->user()->hasPermission('booking_update')): ?>
                                        <a class="dropdown-item"
                    ...
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                        <tr>
                            <td colspan="9" style="text-align:center;padding:60px 20px;color:#aaa;font-size:13px">
                                ...
                                No bookings found.
                                <?php if(Request()->s): ?> Try a different search term. <?php endif; ?>
                            </td>
                        </tr>
                    <?php endif; ?>
                    </tbody>
...
<?php echo $__env->make('admin.layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /home/lionel/Documents/Junkyard/gotrip/bc-cms/modules/Booking/Views/admin/booking/index.blade.php ENDPATH**/ ?>