...
class AdminForm{
    ...
    static function attrsToString(array $attrs): string
    {
    ...
    public static function select2($name,$options,$old = [],$multiple = false,$attrs = []){
        ...
        <select <?php if($multiple)  echo "multiple"; ?> 
            id="select_<?php echo e($name)?>" 
            <?php if(isset($options['required'])) echo 'required'; ?> 
            class="form-control <?php echo e($attrs['class'] ?? 'dungdt-select2-field') ?>" 
            data-options='<?php echo e(json_encode($options['configs'])) ?>' 
            name="<?php echo e($name) ?>"
            <?php echo self::attrsToString($attrs) ?>
            >
            <?php if($multiple): ?>
