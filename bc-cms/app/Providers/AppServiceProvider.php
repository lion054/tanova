...
class AppServiceProvider extends ServiceProvider
{
    ...
    public function boot(Request $request)
    {
        ...
        if (is_installed()) {
            try {
                ...
                // Database tables not ready yet (migrations running)
            }
        ...
        // Vendor API burst limiter — 120 req/min per API key (monthly quota is separate)
        RateLimiter::for('vendor-api', function (Request $req) {
            $key = $req->bearerToken() ?? $req->ip();
    ...
    protected function setLang()
    {
        ...
        try {
            ...
            // Table doesn't exist yet (migrations running)
            $localeCodes = [];
        }
