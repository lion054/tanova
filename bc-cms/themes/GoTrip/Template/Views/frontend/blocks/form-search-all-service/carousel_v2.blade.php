                            ...
                            <div class="tabs__controls d-flex items-center js-tabs-controls">
                                @if ($service_types)
                                    ...
                                    @foreach ($service_types as $service_type)
                                        ...
                                        <div class="">
                                            <button
                                                class="tabs__button px-30 py-20 sm:px-20 sm:py-15 rounded-4 fw-600 text-white js-tabs-button @if ($number == 0) is-tab-el-active @endif"
                                                data-tab-target=".-tab-item-{{ $service_type }}">
                                                @if (!empty($icons[$service_type]))
                            ...
                            <div class="tabs__content js-tabs-content">
                                ...
                                @foreach ($service_types as $service_type)
                                    ...
                                    <div class="tabs__pane -tab-item-{{ $service_type }} @if ($number == 0) is-tab-el-active @endif">
                                        @switch($service_type)
                                            @case('visa')
                                                @livewire($service_type . '::search-form', ['shouldRedirect' => true, 'style' => $style])
                                            @break
                    ...
                    <div class="masthead__images">
                        @if (!empty($list_slider))
                            @foreach ($list_slider as $item)
                                ...
                                <div data-anim-child="slide-up delay-6"><img src="{{ $img }}" alt="image" class="js-mouse-move"
                                        data-move="30"></div>
                            @endforeach
                        @endif
                    </div>
