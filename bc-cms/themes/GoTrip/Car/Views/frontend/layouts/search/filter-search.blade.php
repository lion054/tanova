...
@endif
<form action="{{ url(app_get_locale(false, false, '/') . config('car.car_route_prefix')) }}"
    ...
    <div class="g-filter-item sidebar__item -no-border mb-30 lg:d-none">
        <div class="flex-center ratio ratio-15:9 js-lazy"
            data-bg="{{ get_file_url(setting_item('car_map_image'), 'full') }}">
            <a href="{{ route('car.search', ['_layout' => 'map']) }}"
            ...
            <div class="row x-gap-10 y-gap-30">
                <div class="col-12">
                    <div class="js-price-searchPage">
                        ...
                        <button type="submit"
                            class="flex-center bg-blue-1 rounded-4 px-3 py-1 mt-3 text-12 fw-600 text-white btn-apply-price-range mt-20">{{ __('APPLY') }}</button>
                    </div>
                </div>
            </div>
        </div>
                        ...
                        <div class="col-auto">
                            <div class="d-flex items-center">
                                <div class="form-checkbox ">
                                    <input @if (in_array($number, request()->query('review_score', []))) checked @endif type="checkbox"
                                        name="review_score[]" value="{{ $number }}">
