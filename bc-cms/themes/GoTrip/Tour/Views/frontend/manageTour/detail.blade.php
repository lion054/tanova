...
@section('content')
    <div class="row y-gap-20 justify-between items-end pb-20 lg:pb-40 md:pb-20">
        <div class="col-auto">
            ...
            <div class="text-15 text-light-1">{{ __('AI-native experience management. Live inventory, agent distribution, automated bookings.') }}</div>
        </div>
        <div class="col-auto">
    ...
    <div class="lang-content-box">
        <form action="{{ route('tour.vendor.store', ['id' => $row->id ? $row->id : '-1', 'lang' => request()->query('lang')]) }}"
            method="post">
            @csrf
                ...
                <div class="nav nav-tabs nav-fill" id="nav-tab" role="tablist">
                    ...
                    @if (is_default_lang())
                        <a data-bs-toggle="tab" data-bs-target="#nav-tour-pricing"
                            aria-selected="false">{{ __('3. Pricing') }}</a>
                        ...
                        <a data-bs-toggle="tab" data-bs-target="#nav-attribute"
                            aria-selected="false">{{ __('5. Attributes') }}</a>
                        <a data-bs-toggle="tab" data-bs-target="#nav-ical" aria-selected="false">{{ __('6. Ical') }}</a>
                        <a data-bs-toggle="tab" data-bs-target="#nav-tanova" aria-selected="false">{{ __('7. Tanova') }}</a>
                        <a data-bs-toggle="tab" data-bs-target="#nav-seo" aria-selected="false">{{ __('8. SEO') }}</a>
                    @endif
                </div>
            ...
            <div class="d-flex justify-content-between">
                <button class="button h-50 px-24 -dark-1 bg-blue-1 text-white" type="submit"><i
                        class="fa fa-save mr-2"></i> {{ __('Save Changes') }}</button>
            </div>
        </form>
    </div>
@endsection
