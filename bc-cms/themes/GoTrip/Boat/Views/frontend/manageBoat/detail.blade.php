...
@section('content')
    <div class="row y-gap-20 justify-between items-end pb-20 lg:pb-40 md:pb-20">
        <div class="col-auto">
            ...
            <div class="text-15 text-light-1">{{ __('AI-native boat and charter management. Fleet availability, bookings, and dispatch.') }}</div>
        </div>
    ...
    <div class="lang-content-box">
        <form action="{{ route('boat.vendor.store', ['id' => $row->id ? $row->id : '-1', 'lang' => request()->query('lang')]) }}"
            method="post">
            @csrf
            <div class="form-add-service">
                <div class="nav nav-tabs nav-fill" id="nav-tab" role="tablist">
                    ...
                    @if (is_default_lang())
                        ...
                        <a data-bs-toggle="tab" data-bs-target="#nav-seo" aria-selected="false">{{ __('5. SEO') }}</a>
                    @endif
                </div>
    ...
    <script>
        jQuery(function($) {
            new BCMapEngine('map_content', {
                fitBounds: true,
                center: [{{ $row->map_lat ?? setting_item('map_lat_default', '0') }},
                    {{ $row->map_lng ?? setting_item('map_lng_default', '0') }}
                ],
                zoom: {{ $row->map_zoom ?? '8' }},
