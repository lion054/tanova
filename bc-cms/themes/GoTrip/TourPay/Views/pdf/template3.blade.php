...
<div class="inner">
    <div class="meta-strip" style="display:table;width:100%;background:#f5f5f5;border-radius:4px;padding:14px 18px;margin-bottom:16px;">
        <div class="ms-col"><div class="ms-label">{{ __("From") }}</div><div class="ms-value">{{ $company }}@if($sellerVat)<br><span style="font-size:10px;font-weight:400;color:#888;">{{ __("VAT") }}: {{ $sellerVat }}</span>@endif</div></div>
        <div class="ms-col"><div class="ms-label">{{ __("Billed To") }}</div><div class="ms-value">{{ $row->client_name }}</div></div>
        <div class="ms-col"><div class="ms-label">{{ __("Issued") }}</div><div class="ms-value">{{ $row->issue_date ? $row->issue_date->format('d M Y') : date('d M Y') }}</div></div>
        <div class="ms-col"><div class="ms-label">{{ __("Status") }}</div><div class="ms-value">{{ ucfirst($row->status) }}</div></div>
    </div>
