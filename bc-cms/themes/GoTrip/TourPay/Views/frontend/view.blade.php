...
<div class="tp-view-top">
    <div class="tp-view-top-left">
        ...
        <div class="tp-status-pill tp-pill-{{ $row->status }}">{{ ucfirst($row->status) }}</div>
    </div>
...
<div class="tp-action-bar">
    ...
    <form method="POST" action="{{ route('tourpay.vendor.mark-paid', $row->id) }}" style="display:contents;"
          onsubmit="return confirm('{{ __('Mark this invoice as paid?') }}')">
        @csrf
        <button type="submit" class="tp-ab-btn tp-ab-paid">
            <i class="icofont-check-circled"></i> {{ __("Mark as Paid") }}
        </button>
    </form>
    @endif
...
<div class="tp-doc-frame">
    <div class="tp-doc-inner">
        @include('TourPay::pdf.template' . max(1, min(5, (int) $row->template)), ['row' => $row, 'preview' => true])
    </div>
</div>
