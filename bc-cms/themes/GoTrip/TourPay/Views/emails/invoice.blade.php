...
<div class="wrap">
    ...
    <div class="body">
        <div class="greeting">
            {{ __("Hi :name,", ['name' => $inv->client_name]) }}
        </div>
        <p class="para">
            @if($inv->type === 'quotation')
                {{ __("Please find your quotation from :company attached to this email as a PDF.", ['company' => $company]) }}
            @else
                {{ __("Please find your invoice from :company attached to this email as a PDF.", ['company' => $company]) }}
            @endif
        </p>

        <div class="info-box">
            <div class="info-row"><span class="k">{{ __("Reference") }}</span><span class="v">{{ $inv->invoice_number }}</span></div>
            @if($inv->title)
            <div class="info-row"><span class="k">{{ __("Description") }}</span><span class="v">{{ $inv->title }}</span></div>
            @endif
            <div class="info-row"><span class="k">{{ __("Date") }}</span><span class="v">{{ $inv->issue_date ? $inv->issue_date->format('d M Y') : date('d M Y') }}</span></div>
            @if($inv->due_date)
            <div class="info-row"><span class="k">{{ __("Due Date") }}</span><span class="v">{{ $inv->due_date->format('d M Y') }}</span></div>
            @endif
            <div class="info-row"><span class="k">{{ __("Amount") }}</span><span class="v">{{ $inv->currency }} {{ number_format($inv->total, 2) }}</span></div>
        </div>

        <a href="{{ route('tourpay.pay', $inv->pay_token) }}" class="cta-btn">
            {{ __("View & Pay Online") }}
        </a>

        <p class="para" style="font-size:12px;">
            {{ __("If you have any questions, please don't hesitate to get in touch.") }}
        </p>
    </div>
    <div class="footer">
        {{ $company }} &nbsp;·&nbsp; {{ __("This email was sent by :c", ['c' => $company]) }}
    </div>
</div>
</body>
</html>
