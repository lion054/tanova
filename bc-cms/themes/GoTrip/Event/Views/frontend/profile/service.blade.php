...
@if(!empty($services) and $services->total())
    <div class="bc-profile-list-services">
        ...
        <div class="container">
            ...
            @else
                <div class="text-center mt30"><a class="button h-50 px-24 -dark-1 bg-blue-1 text-white d-inline-flex" href="{{route('user.profile.services',['id'=>$user->user_name ?? $user->id,'type'=>'event'])}}">{{__('View all (:total)',['total'=>$services->total()])}}</a></div>
            @endif
        </div>
    </div>
@endif
