...
<div class="sp-box">
    <div class="sp-nav">
        <a href="{{ route('user.profile.index') }}" class="sp-nav-btn">{{ __("Personal Information") }}</a>
        <a href="{{ route('user.profile.index') }}" class="sp-nav-btn">{{ __("Location") }}</a>
        <span class="sp-nav-btn sp-active">{{ __("Security") }}</span>
    </div>

    <div class="sp-body">
        @include('admin.message')

        <p class="sp-sec">{{ __("Change Password") }}</p>

        <form action="{{ route('user.change_password.update') }}" method="post">
            @csrf
            <div class="sp-f">
                <label class="sp-lbl" for="sp-cur">{{ __("Current Password") }}</label>
                <input class="sp-in" type="password" id="sp-cur" name="current-password" required
                       placeholder="{{ __('Enter your current password') }}">
            </div>
            <div class="sp-f">
                <label class="sp-lbl" for="sp-new">{{ __("New Password") }}</label>
                <input class="sp-in" type="password" id="sp-new" name="new-password" required minlength="8"
                       placeholder="{{ __('At least 8 characters') }}" oninput="spStrength(this.value)">
                ...
                <p class="sp-hint">{{ __("Requires uppercase, lowercase, number and symbol.") }}</p>
            </div>
            <div class="sp-f">
                <label class="sp-lbl" for="sp-conf">{{ __("Confirm New Password") }}</label>
