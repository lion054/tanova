...
@switch($headerStyle)
    ...
    @default
        <header data-add-bg="{{ $dataBg }}" class="header {{ $headerClass }} js-header bc_header style-{{ $headerStyle }}"
            data-x="header" data-x-toggle="is-menu-opened">
            <div data-anim="fade"
                class="{{ $container_class ?? 'header__container' }} px-30 sm:px-20 @if ($headerStyle == 'transparent_v2') container @endif is-in-view">
                <div class="row justify-between items-center">
                    <div class="col-auto @if ($headerStyle == 'transparent_v7') col-left @endif">
                        ...
                        @elseif($headerStyle == 'transparent_v4')
                            @include('Layout::parts.header-style.transparent_v2', [
                    ...
                    @if ($headerStyle == 'transparent_v3')
                        <div class="col-auto xl:d-none">
                            <a href="{{ url(app_get_locale(false, '/')) }}" class="header-logo mr-20" data-x="header-logo"
                    ...
                    <div class="col-auto">
                                            ...
                                            @if (!Auth::check())
                                                <div class="d-flex items-center ml-20 is-menu-opened-hide md:d-none">
                                                    ...
                                                    @if (!empty(($page_vendor = get_page_url(setting_item('vendor_page_become_an_expert')))))
                                                        <a href="{{ $page_vendor }}"
                                                            class="{{ $btn_expert }} button px-30 fw-400 text-14  h-50">{{ __('Become An Expert') }}</a>
                                                    @endif
                                                    <a data-bs-toggle="modal" href="#login"
                                                        class="{{ $btn_login }} button px-30 fw-400 text-14  h-50 ml-20">{{ __('Sign In / Register') }}</a>
                                                </div>
                                            @else
                                                    ...
                                                    <a href="#" class="is_login">
                                                        <span class="mr-10">
                                                            @if ($avatar_url = Auth::user()->getAvatarUrl())
                                                                <img class="avatar rounded-circle" src="{{ $avatar_url }}"
                                                                    alt="{{ Auth::user()->getDisplayName() }}" width="30"
                                                                    height="30">
                                                            @else
                                                                <span
                                                                    class="avatar-text rounded-circle">{{ ucfirst(Auth::user()->getDisplayName()[0]) }}</span>
                                                            @endif
                                                        ...
                                                        <li class="menu-hr">
                                                            <a class="dropdown-item" href="#"
                                                                onclick="event.preventDefault(); document.getElementById('logout-form').submit();"><i
                                                                    class="fa fa-sign-out mr-10"></i> {{ __('Logout') }}</a>
                                                        </li>
                                                    </ul>
                        ...
                        <div class="d-none xl:d-flex x-gap-20 items-center pl-30 {{ $navTextStyle }}"
                            data-x="header-mobile-icons" data-x-toggle="text-white">
                            <div><a href="@if (!Auth::check()) {{ url('/login') }} @else {{ route('user.profile.index') }} @endif"
