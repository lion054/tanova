...
@section('content')
    <div class="row y-gap-20 justify-between items-end pb-60 lg:pb-40 md:pb-32">
        <div class="col-auto">
            ...
            <div class="text-15 text-light-1">{{ __('AI-native property management. Live inventory, smart pricing, automated operations.') }}</div>
        </div>
        <div class="col-auto">
            @if($hotel)
                            ...
                            <ul class="nav nav-tabs  flex-column vertical-nav" id="items_tab" role="tablist">
                                @foreach ($rows as $k => $item)
                                    <li class="nav-item event-name ">
                                        <a class="nav-link" data-id="{{ $item->id }}" data-bs-toggle="tab"
                                            data-bs-target="#calendar-{{ $item->id }}"
...
@push('js')
    ...
    <script>
        ...
        const loadDateRoute = '{{ $hotel ? route('hotel.vendor.room.availability.loadDates', ['hotel_id' => $hotel->id]) : route('hotel.vendor.availability.loadDates') }}';
        const storeDateRoute = '{{ $hotel ? route('hotel.vendor.room.availability.store', ['hotel_id' => $hotel->id]) : route('hotel.vendor.availability.store') }}';
