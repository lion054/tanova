                ...
                <div class="col-md-3">
                    <div class="nav flex-column nav-pills" id="v-pills-tab" role="tablist" aria-orientation="vertical">
                        @foreach ($this->steps as $index => $step)
                            <button class="border-0 nav-link {{ $index == $this->stepIndex ? 'active' : '' }} mb-2"
                                @if ($index > $this->maxStepIndex) disabled @elseif($index !== $this->stepIndex) wire:click="setStep('{{ $index }}')" @endif
                                id="{{ $step['id'] }}-tab" type="button" role="tab"
                ...
                <div class="col-md-9">
                    @if ($this->currentStep)
                        ...
                        <div class="d-flex justify-content-center">
                            <button
                                class="button h-60 px-24 -dark-1 bg-blue-1 text-white">{{ __('Next Step') }}</button>
                        </div>
                    @endif
                </div>
            </div>
