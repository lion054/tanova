...
@endpush
<div class="container">
    <div class="bc-vendor-form-register layout-pt-lg @if (!empty($layout)) {{ $layout }} @endif">
        <div class="row">
            <div class="col-12 col-lg-5">
                <h1 class="mb-10">{{ $title }}</h1>
                <p class="sub-heading mb-30">{{ $desc }}</p>
                <form class="form bc-form-register-vendor" method="post" action="{{ route('vendor.register') }}">
                    @csrf
                ...
                <div class="bc_gallery">
                    <div class="btn-group">
                        <span class="btn-transparent has-icon bc-video-popup"
                            @if (!empty($youtube)) data-toggle="modal" @endif
                            data-src="{{ handleVideoUrl($youtube) }}" data-target="#video-register">
                            @if ($bg_image)
...
@push('js')
    <script type="text/javascript" src="{{ asset('/module/vendor/js/vendor-register.js?_ver=' . config('app.version')) }}">
    </script>
@endpush
