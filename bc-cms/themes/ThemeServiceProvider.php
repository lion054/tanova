...
class ThemeServiceProvider extends ServiceProvider
{
    ...
    protected function loadModuleViews($theme)
    {
        ...
        foreach ($listModule as $module) {
            ...
            // Load style module views if style is not default
            if ($style and $style !== 'default') {
                if (is_dir(base_path('themes/' . ucfirst($theme) . '/Styles/' . ucfirst($style) . '/' . $module))) {
                    $this->loadViewsFrom(base_path('themes/' . ucfirst($theme) . '/Styles/' . ucfirst($style) . '/' . $module) . '/Views', $module);
                }
            }
        ...
        // Load style module views if style is not default
        if ($style and $style !== 'default') {
            if (is_dir(base_path('themes/' . ucfirst($theme) . '/Styles/' . ucfirst($style) . '/Layout'))) {
                $this->loadViewsFrom(base_path('themes/' . ucfirst($theme) . '/Styles/' . ucfirst($style) . '/Layout'), 'Layout');
            }
        }
