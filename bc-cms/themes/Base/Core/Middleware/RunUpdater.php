...
use Themes\Base\Core\Updaters\Updater362;
use Themes\Base\Core\Updaters\Updater401;
...
<a href="mailto:contact@bookingcore.com">contact@bookingcore.com</a>',
    ...
    public function updateTo151()
    {
        ...
        foreach ($allServices as $k=>$service) {
            if(in_array($k,['plan', 'tanova_trip'])) continue;
    ...
    public function updateTo220()
    {
        ...
        Schema::table('bc_bookings', function (Blueprint $table) {
            if (!Schema::hasColumn('bc_bookings', 'is_paid')) {
    ...
    public function updateTo240()
    {
        ...
        DB::statement('ALTER TABLE bc_coupons MODIFY only_for_user varchar(191)');
        ...
        Schema::table('bc_boats', function (Blueprint $table) {
            if (!Schema::hasColumn('bc_boats', 'include')) {
                $table->text('include')->nullable();
            }
            if (!Schema::hasColumn('bc_boats', 'exclude')) {
                $table->text('exclude')->nullable();
            }
        });
        Schema::table('bc_boat_translations', function (Blueprint $table) {
            if (!Schema::hasColumn('bc_boat_translations', 'include')) {
                $table->text('include')->nullable();
            }
            if (!Schema::hasColumn('bc_boat_translations', 'exclude')) {
                $table->text('exclude')->nullable();
            }
        });
    ...
    {
        $all = Review::query()->whereNull('object_author_id')->get();
        if (!empty($all)) {
            foreach ($all as $item) {
                switch ($item->object_model) {
                    case "tour":
                        ...
                        if ($tour) {
                            $item->object_author_id = $tour->author_id;
                            $item->save();
                        }
                        break;
                    case "space":
