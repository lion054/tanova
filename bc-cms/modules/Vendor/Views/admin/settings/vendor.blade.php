        ...
        <div class="panel">
            <div class="panel-body">
                @if(is_default_lang())
                    ...
                    <div class="form-group">
                        <label>{{__('Company sign-up')}}</label>
                        <div class="form-controls">
                            ...
                            <div class="form-group">
                                ...
                                <select name="vendor_signup_plan_id" class="form-control" style="max-width:320px;">
                                    ...
                                    @foreach(\Modules\Vendor\Models\VendorPlan::where('status', 'publish')->orderBy('price')->get() as $plan)
                                        <option value="{{ $plan->id }}" @if(($settings['vendor_signup_plan_id'] ?? '') == $plan->id) selected @endif>{{ $plan->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
...
<div class="row">
    <div class="col-sm-4">
        <h3 class="form-group-title">{{__('Subscription Settings')}}</h3>
        <p class="form-group-desc">{{__('Control how vendor subscription plans are enforced on this platform.')}}</p>
    </div>
    <div class="col-sm-8">
        <div class="panel">
            <div class="panel-body">
                <div class="form-group">
                    <label>
                        <input type="checkbox" name="vendor_subscription_enabled" value="1"
                            @if(setting_item('vendor_subscription_enabled') == 1) checked @endif>
                        {{__("Require vendors to have an active subscription plan?")}}
                    </label>
                    <p><i>{{__('When enabled, vendors without an active plan will be redirected and cannot manage listings.')}}</i></p>
                </div>
                <div class="form-group" data-condition="vendor_subscription_enabled:is(1)">
                    <label>{{__('Grace Period (days after expiry)')}}</label>
                    <div class="form-controls">
                        <input type="number" class="form-control" name="vendor_subscription_grace_period"
                            value="{{setting_item('vendor_subscription_grace_period', 0)}}"
                            min="0" max="90" style="width:120px">
                    </div>
                    <p><i>{{__('Vendors retain access for this many days after their subscription expires. Set 0 for immediate lockout.')}}</i></p>
                </div>
                <div class="form-group" data-condition="vendor_subscription_enabled:is(1)">
                    <label>{{__('Redirect expired vendors to (URL or route)')}}</label>
                    <div class="form-controls">
                        <input type="text" class="form-control" name="vendor_subscription_expired_redirect"
                            value="{{setting_item('vendor_subscription_expired_redirect')}}"
                            placeholder="{{__('e.g. /vendor/subscription or leave blank for dashboard')}}">
                    </div>
                </div>
                <div class="form-group mt-3">
                    <a href="{{route('vendor.admin.plan.index')}}" class="btn btn-info btn-sm">
                        <i class="fa fa-list"></i> {{__('Manage Plans')}}
                    </a>
                    <a href="{{route('vendor.admin.subscription.index')}}" class="btn btn-secondary btn-sm ml-2">
                        <i class="fa fa-users"></i> {{__('View All Subscriptions')}}
                    </a>
                </div>
            </div>
        </div>
