<tr>
    ...
    <td width="2%">
        ...
        @if(!empty(setting_item("visa_allow_vendor_can_change_their_booking_status")))
            ...
            <div class="dropdown-menu">
                @if(!empty($statues))
                    @foreach($statues as $status)
                        <a href="{{ route("visa.vendor.booking_report.bulk_edit" , ['id'=>$booking->id , 'status'=>$status]) }}">
        ...
        @endif
        @if(!empty(setting_item("visa_allow_vendor_can_change_paid_amount")))
            <a class="btn btn-xs btn-info btn-make-as w-100" data-toggle="modal" data-target="#modal-paid-{{$booking->id}}">
                <i class="fa fa-dollar"></i>{{__("Set Paid")}}
            </a>
            @include ($service->set_paid_modal_file ?? '')
        @endif
        <a href="{{route('visa.user.booking-detail',['code'=>$booking->code,'source'=>'report'])}}" class="btn btn-xs btn-primary mt-1 btn-info-booking px-1 d-inline-flex align-items-center" style="width: auto">
