...
class UserController extends Controller
{
    ...
    public function addWishList(Request $request, $objectModel, $objectId)
    {
        $class = app(\Modules\User\Controllers\UserWishListController::class);

        $request->merge(['object_model' => $objectModel, 'object_id' => $objectId, 'action' => 'add']);

        return $class->handleWishList($request);
    }

    public function deleteWishList(Request $request, $objectModel, $objectId)
    {
        $class = app(\Modules\User\Controllers\UserWishListController::class);

        $request->merge(['object_model' => $objectModel, 'object_id' => $objectId]);

        $class->handleRemove($request);
    ...
    public function deleteWishListAll(Request $request)
    {
        app(UserWishList::class)->query()
            ->where("user_wishlist.user_id",Auth::id())
            ->delete();
        return $this->sendSuccess();
    }
