...
trait HasUpload
{
    ...
    public function uploadFile(Request $request,$fileName = 'file',$fileType = 'image',$folder_id = 0,$options = [])
    {
        ...
        return $this->uploadSingleFile($file,$folder_id,$options);

    }

    public function uploadSingleFile(UploadedFile $file,$folder_id = 0,$options = [])
    {
        ...
        if(!empty($options['folder_subfix'])){
            $folder .= $options['folder_subfix'].'/';
        }

        $shouldHashFileName = isset($options['should_hash_file_name']) ? $options['should_hash_file_name'] : false;
        ...
        if(!$shouldHashFileName){
            ...
            $newFileName2 = $newFileName2 . '.' . $file->getClientOriginalExtension();
        }else{
            $newFileName2 = $file->hashName();
        }

        if(in_array($driver,['local','uploads'])){
            $check = $file->storePubliclyAs( $folder, $newFileName2,$driver);
        }else{
            $check = $file->storeAs( $folder, $newFileName2,$driver);
        }
        ...
        if ($check) {
            try {
                ...
                $fileObj->file_name = $shouldHashFileName ? $newFileName : $newFileName2;
