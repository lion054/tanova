...
class Space extends Bookable
{
    ...
    public function addToCart(Request $request)
    {
        ...
        //Default by day
        $total_day_or_night = $interval->days + 1;
        ...
        if (!empty($this->discount_by_days)) {
            foreach ($this->discount_by_days as $type) {
                if ($type['from'] <= $total_day_or_night and (!$type['to'] or $type['to'] >= $total_day_or_night)) {
                    $type_total = 0;
                    switch ($type['type']) {
                        case "fixed":
        ...
        if ($check) {
            ...
            $booking->addMeta('discount_by_days', $discount_by_days ?? []);
            if ($this->isDepositEnable()) {
    ...
    public function getBookingData()
    {
        ...
        $booking_data = [
            ...
            'discount_by_days' => $this->discount_by_days ?? [],
        ];
        if (!empty($adults = request()->input('adults'))) {
    ...
    {
        return $this->reviewClass::select(['id', 'title', 'content', 'rate_number', 'author_ip', 'status', 'created_at', 'object_author_id', 'author_id'])->where('object_id', $this->id)->where('object_model', 'space')->where("status", "approved")->orderBy("id", "desc")->with('author')->paginate(setting_item('space_review_number_per_page', 5));
    }
    ...
    {
        $query = parent::query()->select("bc_spaces.*");
        $query->where("bc_spaces.status", "publish");
        ...
        if ($term_id = $request['term_id'] ?? "") {
            $query->join('bc_space_term as tt1', function ($join) use ($term_id) {
                $join->on('tt1.target_id', "bc_spaces.id");
                $join->where('tt1.term_id', $term_id);
            });
        }

        if (!empty($request['attrs'])) {
            $this->filterAttrs($query, $request['attrs'], 'bc_space_term');
        }
        ...
        if (!empty($service_name = $request['service_name'] ?? "")) {
            if (setting_item('site_enable_multi_lang') && setting_item('site_locale') != app()->getLocale()) {
                $query->leftJoin('bc_space_translations', function ($join) {
                    $join->on('bc_spaces.id', '=', 'bc_space_translations.origin_id');
                });
                $query->where('bc_space_translations.title', 'LIKE', '%' . $service_name . '%');
            } else {
                $query->where('bc_spaces.title', 'LIKE', '%' . $service_name . '%');
            }
        }
