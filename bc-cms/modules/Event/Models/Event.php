...
class Event extends Bookable
{
    ...
    {
        return $this->reviewClass::select(['id', 'title', 'content', 'rate_number', 'author_ip', 'status', 'created_at', 'object_author_id', 'author_id'])->where('object_id', $this->id)->where('object_model', 'event')->where("status", "approved")->orderBy("id", "desc")->with('author')->paginate(setting_item('event_review_number_per_page', 5));
    }
    ...
    {
        $query = parent::query()->select("bc_events.*");
        $query->where("bc_events.status", "publish");
        ...
        if ($term_id = $request['term_id'] ?? "") {
            $query->join('bc_event_term as tt1', function ($join) use ($term_id) {
                $join->on('tt1.target_id', "bc_events.id");
                $join->where('tt1.term_id', $term_id);
            });
        }

        if (!empty($request['attrs'])) {
            $this->filterAttrs($query, $request['attrs'], 'bc_event_term');
        }
        ...
        if (!empty($service_name = $request["service_name"] ?? "")) {
            if (setting_item('site_enable_multi_lang') && setting_item('site_locale') != app()->getLocale()) {
                $query->leftJoin('bc_event_translations', function ($join) {
                    $join->on('bc_events.id', '=', 'bc_event_translations.origin_id');
                });
                $query->where('bc_event_translations.title', 'LIKE', '%' . $service_name . '%');
            } else {
                $query->where('bc_events.title', 'LIKE', '%' . $service_name . '%');
            }
        }
        ...
        switch ($orderby) {
            ...
            default:
                if (!empty($request['order']) and !empty($request['order_by'])) {
                    $query->orderBy("bc_events." . $request['order'], $request['order_by']);
                } else {
    ...
    public function scopeForVendor(\Illuminate\Database\Eloquent\Builder $query, ?int $vendorId = null): \Illuminate\Database\Eloquent\Builder
    {
        $id = $vendorId ?? \App\Services\VendorContext::id();
        return $id ? $query->where('author_id', $id) : $query->whereRaw('0=1');
    }
}
