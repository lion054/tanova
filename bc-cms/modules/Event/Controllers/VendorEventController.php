...
class VendorEventController extends FrontendController
{
    ...
    public function restore($id)
    {
        ...
        $user_id = Auth::id();
        $query = $this->eventClass::onlyTrashed()->where("author_id", $user_id)->where("id", $id)->first();
        if(!empty($query)){
            $query->restore();
        }
    ...
    public function deleteEvent($id)
    {
        ...
        if(\request()->query('permanently_delete')){
            $query = $this->eventClass::where("author_id", $user_id)->where("id", $id)->withTrahsed()->first();
            if (!empty($query)) {
                $query->forceDelete();
            }
        }else {
