...
class Ledger
{
    public const KINDS = ['payment', 'refund', 'expense', 'payout'];
    ...
    public function record(array $a): LedgerEntry
    {
        ...
        [$base, $baseAmount] = $this->inBase((int) $a['vendor_id'], $amount, $currency);
        ...
        try {
            $entry = LedgerEntry::withoutVendorScope()->create([
                ...
                'reverses_id' => $a['reverses_id'] ?? null, 'reference' => isset($a['reference']) ? substr((string) $a['reference'], 0, 120) : null,
    ...
    {
        return max(0.0, round((float) LedgerEntry::withoutVendorScope()->where('booking_id', $bookingId)->whereIn('kind', ['payment', 'refund'])->sum('amount'), 2));
    }
    ...
    public function platformHeldByBooking(int $vendorId): array
    {
        $rows = LedgerEntry::withoutVendorScope()->where('vendor_id', $vendorId)->whereIn('kind', ['payment', 'refund'])->whereNotNull('booking_id')
            ->groupBy('booking_id')->selectRaw("booking_id, SUM(amount) AS everything, SUM(CASE WHEN held_by = 'platform' THEN amount ELSE 0 END) AS platform")->get();
        $out = [];
        foreach ($rows as $r) {
     ...
     * reference); it never goes over what the invoice still owes, nor below what was allocated.
     */
    public function allocateToInvoice(LedgerEntry $entry, ?Booking $booking = null): ?Payment
    {
        ...
        if (!$inv || strtoupper((string) $inv->currency) !== $entry->currency) {
            return null;
        }
        ...
        if ($signed > 0) {
            ...
            $allocated = round((float) (clone $confirmed)->where('source', 'booking')->sum('amount'), 2);
            $amount = -min(-$signed, max(0.0, $allocated));
        }
        if (abs($amount) < 0.005) {
            return null;
        }
        $p = Payment::withoutVendorScope()->create(['vendor_id' => $inv->vendor_id, 'invoice_id' => $inv->id, 'amount' => $amount, 'method' => $entry->method === 'card' || $entry->held_by === 'platform' ? 'card' : ($entry->method ?: 'other'),
            'reference' => $entry->reference, 'gateway_ref' => $entry->entry_key, 'paid_at' => $entry->occurred_at->toDateString(), 'notes' => $amount < 0 ? __('Refunded through the booking') : __('Paid through the booking'), 'source' => 'booking', 'status' => 'confirmed']);
        $inv->recalculate();

        return $p;
    }

    private function inBase(int $vendorId, float $amount, string $currency): array
    {
        try {
            ...
            return [$base, $settings->toBase($amount, $currency)];
        } catch (\Throwable $e) {
            return [null, null];
        }
    }
