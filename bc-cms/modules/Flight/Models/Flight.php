...
class Flight extends Bookable
{
    ...
    {
        return $this->reviewClass::select(['id', 'title', 'content', 'rate_number', 'author_ip', 'status', 'created_at', 'object_author_id', 'author_id'])->where('object_id', $this->id)->where('object_model', 'Flight')->where("status", "approved")->orderBy("id", "desc")->with('author')->paginate(setting_item('flight_review_number_per_page', 5));
    }
    ...
    public function search($request)
    {
        ...
        $query = self::query()->select(['bc_flight.*'])->where('status', 'publish');
        ...
        if (!empty($request['attrs'])) {
            $this->filterAttrs($query, $request['attrs'], 'bc_flight_term');
        }
    ...
    public static function searchCustom(Request $request)
    {
        ...
        if (!empty($lat = $request->query('map_lat')) and !empty($lgn = $request->query('map_lng'))) {
