            ...
            <div class="col-md-4 mb40">
                <div class="panel">
                    ...
                    <div class="panel-body">
                        <form action="{{route('support.admin.topic.category.store',['id'=>-1])}}" method="post">
                            @csrf
                            @include('Support::admin/topic/category/form',['parents'=>$rows])
                            <div class="">
            ...
            <div class="col-md-8">
                <div class="filter-div d-flex justify-content-between ">
                    <div class="col-left">
                        @if(!empty($rows))
                            <form
                                method="post" action="{{route('support.admin.topic.category.bulkEdit')}}"
                    ...
                    <div class="col-left">
                        <form
                            method="get"
                            action="{{route('support.admin.topic.category.index')}} "
                            ...
                            <table class="table table-hover">
                                ...
                                @if(count($rows) > 0)
                                    ...
                                    <tr>
                                        ...
                                        <td class="title">
                                            <a href="{{route('support.admin.topic.category.edit',['id'=>$row->id])}}">{{$prefix.' '.$row->name}}</a>
                                        </td>
                                        <td>
                                            @if($row->image_id)
                                                <img src="{{get_file_url($row->image_id)}}" width="90" alt="">
                                            @endif
                                        </td>
                                        <td>
                                            {{$row->display_order}}
                                        ...
                                        <td>
                                            <a
                                                class="btn btn-sm btn-default" href="{{route('support.admin.topic.category.edit',['id'=>$row->id])}}"
                                            >{{__("Edit")}}</a>
                                        </td>
                                    </tr>
                                    <?php
