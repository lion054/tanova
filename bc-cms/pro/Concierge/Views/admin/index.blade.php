...
<div class="container-fluid">
    ...
    <div class="d-flex justify-content-between align-items-start mb-4 flex-wrap gap-3">
        ...
        <div class="d-flex gap-2 flex-wrap">
            <a href="{{ route('admin.concierge.create') }}" class="tp-ab-btn tp-ab-btn--primary">
    ...
    <div class="cc-stats">
        ...
        <div class="tp-stat tp-stat--green">
            ...
            <div class="tp-stat__amount green">{{ $cntOpen }}</div>
        ...
        <div class="tp-stat tp-stat--red">
            ...
            <div class="tp-stat__amount red">{{ $cntEscalated }}</div>
        ...
        <div class="tp-stat tp-stat--gray">
            ...
            <div class="tp-stat__amount gray">{{ $cntResolved }}</div>
            <div class="tp-stat__sub">closed conversations</div>
        </div>
    </div>

    {{-- Filter --}}
    @php $curStatus = request('status',''); $curChannel = request('channel',''); $curSearch = request('s',''); @endphp
    <form method="GET" class="cc-filter">
        <div class="cc-search">
            <i class="ion ion-ios-search" style="color:#ccc"></i>
            <input type="text" name="s" value="{{ $curSearch }}" placeholder="Search guest name or email…">
        </div>
        <select name="channel" class="cc-select" onchange="this.form.submit()">
            <option value="">All Channels</option>
            <option value="web"      @selected($curChannel==='web')>Web</option>
            <option value="whatsapp" @selected($curChannel==='whatsapp')>WhatsApp</option>
            <option value="email"    @selected($curChannel==='email')>Email</option>
            <option value="phone"    @selected($curChannel==='phone')>Phone</option>
        </select>
        <div class="cc-chips">
            <a href="?" class="cc-chip {{ $curStatus==='' ? 'active':'' }}">
                All <span class="n">{{ $cntAll }}</span>
            </a>
            <a href="?status=open{{ $curChannel ? '&channel='.$curChannel : '' }}"
               class="cc-chip {{ $curStatus==='open' ? 'active':'' }}">
                Open <span class="n">{{ $cntOpen }}</span>
            </a>
            <a href="?status=escalated{{ $curChannel ? '&channel='.$curChannel : '' }}"
               class="cc-chip {{ $curStatus==='escalated' ? 'active':'' }}">
                Escalated <span class="n">{{ $cntEscalated }}</span>
            </a>
            <a href="?status=resolved{{ $curChannel ? '&channel='.$curChannel : '' }}"
               class="cc-chip {{ $curStatus==='resolved' ? 'active':'' }}">
                Resolved <span class="n">{{ $cntResolved }}</span>
            </a>
        </div>
        @if($curSearch)<button type="submit" class="tp-ab-btn tp-ab-btn--ghost">Search</button>@endif
    </form>
                ...
                <tr>
                    <td>
                        <div style="display:flex;align-items:center;gap:10px">
                            <div class="cc-avatar">{{ strtoupper(substr($conv->guestDisplayName(), 0, 1)) }}</div>
                            <div>
                                <a href="{{ route('admin.concierge.show', $conv) }}"
                                   style="font-weight:600;color:#0a0a0a;text-decoration:none">
                                    {{ $conv->guestDisplayName() }}
                                </a>
                                @if($conv->guest_email)
                                <div style="font-size:11px;color:#aaa">{{ $conv->guest_email }}</div>
                                @endif
                            </div>
                        </div>
                    </td>
                    <td>
                        ...
                        @endphp
                        <span class="tp-badge {{ $chBadge }}">{{ ucfirst($conv->channel) }}</span>
                    </td>
                    ...
                    <td style="color:#aaa;font-size:12px;white-space:nowrap">{{ $conv->last_message_at?->diffForHumans() }}</td>
                    <td>
                        <a href="{{ route('admin.concierge.show', $conv) }}" class="tp-ab-btn tp-ab-btn--outline">
                            Open
                        </a>
                    </td>
                </tr>
