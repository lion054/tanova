...
<div class="container-fluid">
    ...
    <div class="d-flex justify-content-between align-items-start mb-4 flex-wrap gap-3">
        <div>
            ...
            <h1 class="portal-h1">Tanova <em>— {{ $hours }}h window</em></h1>
        </div>
        <div class="d-flex gap-2 flex-wrap">
            <form method="POST" action="{{ route('admin.tanova.expire-old') }}">
                @csrf
                <button class="tp-ab-btn tp-ab-btn--ghost"
                        onclick="return confirm('Delete all created trips older than {{ $hours }}h?')">
    ...
    <div class="tn-stats">
        <div class="tp-stat tp-stat--default">
            ...
            <div class="tp-stat__amount">{{ $cntAll }}</div>
            <div class="tp-stat__sub">trips in last {{ $hours }}h</div>
        </div>
        <div class="tp-stat tp-stat--amber">
            <div class="tp-stat__label"><i class="ion ion-ios-time"></i> Awaiting</div>
            <div class="tp-stat__amount amber">{{ $cntCreate }}</div>
        ...
        <div class="tp-stat tp-stat--green">
            ...
            <div class="tp-stat__amount green">{{ $cntBooked }}</div>
        ...
        <div class="tp-stat tp-stat--blue">
            ...
            <div class="tp-stat__amount blue">{{ $hours }}h</div>
    ...
    <div class="tn-gen-drawer" id="tn-gen-drawer">
        ...
        <form method="POST" action="{{ route('admin.tanova.generate') }}">
            @csrf
            ...
            <div class="row g-3">
                ...
                <div class="col-md-1">
                    <div class="tp-field">
                        <label>Guests</label>
                        <input type="number" name="guests" value="{{ old('guests', 2) }}" min="1" max="50" required>
                    </div>
                </div>
                <div class="col-md-2">
                    <div class="tp-field">
                        <label>Budget (USD total)</label>
                        <input type="number" name="budget" value="{{ old('budget', 2000) }}" min="100" step="50" required>
                    </div>
                </div>
                <div class="col-md-2">
                    <div class="tp-field">
    ...
    {{-- Filter --}}
    @php $curStatus = request('status',''); $curDest = request('destination',''); @endphp
    <div class="tn-filter">
        <form method="GET" class="d-flex gap-2 align-items-center flex-wrap" style="flex:1">
            <div class="tn-search">
                <i class="ion ion-ios-search"></i>
                <input type="text" name="destination" value="{{ $curDest }}" placeholder="Search destination…">
                @if($curStatus)<input type="hidden" name="status" value="{{ $curStatus }}">@endif
            </div>
            <div class="tn-chips">
                <a href="?" class="tn-chip {{ $curStatus==='' ? 'active':'' }}">
                    All <span class="n">{{ $cntAll }}</span>
                </a>
                <a href="?status=created" class="tn-chip {{ $curStatus==='created' ? 'active':'' }}">
                    Awaiting <span class="n">{{ $cntCreate }}</span>
                </a>
                <a href="?status=booked" class="tn-chip {{ $curStatus==='booked' ? 'active':'' }}">
                    Booked <span class="n">{{ $cntBooked }}</span>
                </a>
            </div>
            @if($curDest)
    ...
    <div class="tp-card">
        <table class="tp-table">
            ...
            <tbody>
                ...
                <tr>
                    <td>
                        <a href="{{ route('admin.tanova.show', $trip) }}"
                    ...
                    <td>
                        @if($trip->guest_name)
                            <div style="font-weight:600;font-size:13px">{{ $trip->guest_name }}</div>
                            @if($trip->guest_email)
                            <div style="font-size:11px;color:#aaa">{{ $trip->guest_email }}</div>
                            @endif
                            @if($trip->guest_phone)
                            <div style="font-size:11px;color:#aaa">{{ $trip->guest_phone }}</div>
                            @endif
                        @else
                    ...
                    <td>
                        ...
                        <div style="font-size:11px;color:#aaa">{{ $trip->nightCount() }} nights</div>
                    </td>
                    <td>{{ $trip->guests }}</td>
                    <td style="font-weight:600">
                        {{ number_format($trip->estimated_price, 0) }}
                        <span style="font-size:11px;color:#aaa;font-weight:400"> {{ $trip->currency }}</span>
                    </td>
                    <td>
                    ...
                    <td style="color:#aaa;font-size:12px">{{ $trip->created_at?->diffForHumans() }}</td>
                    <td>
                        <div class="d-flex gap-1">
                            <a href="{{ route('admin.tanova.show', $trip) }}" class="tp-ab-btn tp-ab-btn--outline">
                                View
                            </a>
                            @if($trip->isMovable())
                            <form method="POST" action="{{ route('admin.tanova.move', $trip) }}" style="display:inline">
                                @csrf
                ...
                <tr>
                    <td colspan="9">
                        <div class="tp-empty">
                            ...
                            No Tanova trips in the last {{ $hours }} hours.
        ...
        @if($trips->hasPages())
        <div style="padding:14px 16px;border-top:1px solid #f0f0f0">{{ $trips->withQueryString()->links() }}</div>
        @endif
    </div>
