Vue.component('bc-messages-box', {
    template: '#bc-messages-box',
    data() {
        return {
        }
    },
...
window.bookingCoreChatBox = new Vue({
    el:'#bc-chat-box',
    data:{
        config:typeof bc_chat_config !='undefined' ? bc_chat_config : {
    ...
    methods:{
        setChatWindow:function(open){
            ...
            if(window.localStorage){
                window.localStorage.setItem('bc_inbox_maximum',open ? 1 : 0);
            }
        },
        openConversation:function(chat){
            ...
            if(open && window.localStorage){
                window.localStorage.setItem('bc_inbox_last',chat.id);
            }
    ...
    created:function () {
        ...
        if(this.config.enable &&  bookingCore.currentUser){
            ...
            if(window.localStorage){
                var bc_inbox_last = window.localStorage.getItem('bc_inbox_last');
                if(bc_inbox_last && bc_inbox_last != 'undefined'){
                    me.currentConversation.id = bc_inbox_last;
            ...
            me.getChatNofitications(function (json) {
                if(window.localStorage){
                    var bc_inbox_maximum = window.localStorage.getItem('bc_inbox_maximum');
                    if(bc_inbox_maximum > 0){
                        me.setChatWindow(true);
                    }
                }
