function goTripReloadForm(url = "") {
  ...
  $(".map_loading").show();
  $.ajax({
    data: $('.bc_form_search_map input,select,textarea,input:hidden[value!=""],#advance_filters input,select,textarea').serialize() + "&_ajax=1",
    url: action,
    dataType: "json",
    type: "get",
...
$(".boat_form_search_map .bc_form").submit(function (e) {
  e.preventDefault();
  goTripReloadForm();
});
...
$(document).on("click", ".bc-review-score .button", function () {
...
$(document).on("click", ".bc-order .dropdown-item", function () {
  let order = $(this).attr("data-order");
  $(".orderby").val(order);
  goTripReloadForm();
});

$(document).on("click", ".goTrip-bc-pagination a", function (e) {
  e.preventDefault();
