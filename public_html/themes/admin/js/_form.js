...
(function ($) {
    ...
    window.initEditorFor = function (el){
        ...
        tinymce.init({
            ...
            toolbar: 'formatselect | bold italic strikethrough forecolor backcolor permanentpen formatpainter blockquote | link image media | alignleft aligncenter alignright alignjustify  | numlist bullist outdent indent  | pagebreak codesample code | removeformat',
            image_advtab: false,
            image_caption: false,
    ...
    $(document).on('click','.dungdt-upload-box-normal .btn-field-upload,.dungdt-upload-box-normal .attach-demo,.dungdt-upload-file .btn-field-upload',function () {
        let p = $(this).closest('.dungdt-upload-box');
        let type = $(this).data("type");
        void 0 !== type && type || (type = "image");
        uploaderModal.show({
            multiple: false,
            file_type: type,
            onSelect: function (files) {
                ...
                p.find('.input-name').val(files[0].full_size);
            },
        });
    });
